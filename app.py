...
DATABASE = os.getenv("DB_NAME", "fireguard_db")
...
def get_lan_ip():
    """Best-effort LAN IP of this machine, for display only (e.g. telling the
    admin what Server URL to type into the agent installer on another PC)."""
...
def detect_repeated_connection_attempts(ip, count):
    ...
    recent = fetch_one(
        ...
        WHERE alert_type = 'Possible Repeated Connection Attempt' AND ip_address = ?
          AND alert_time >= DATEADD(SECOND, ?, GETDATE())
...
def _register_failed_login(user):
    ...
    if refreshed and refreshed["failed_attempts"] >= MAX_LOGIN_ATTEMPTS:
        execute(
            "UPDATE users SET locked_until = DATEADD(MINUTE, ?, GETDATE()) WHERE user_id = ?",
            [LOGIN_LOCKOUT_MINUTES, user["user_id"]],
        )
        return True
    return False
...
def login():
    if request.method == "POST":
        ...
        if user and user.get("locked_until") and user["locked_until"] > datetime.now():
            ...
            minutes_left = max(1, math.ceil(remaining.total_seconds() / 60))
            flash(
...
def dashboard():
    ...
    rules = fetch_all(
        """
        SELECT TOP 6 rule_id, rule_name, ip_address, port, protocol, action_type, status
        FROM firewall_rules
    ...
    trend_rows = fetch_all(
        """
        SELECT DATEPART(HOUR, log_time) AS hr, COUNT(*) AS total
        FROM traffic_logs
        WHERE log_time >= DATEADD(HOUR, -24, GETDATE())
    ...
    execute(
        ...
        WHERE last_seen IS NULL
           OR last_seen < DATEADD(SECOND, -{AGENT_OFFLINE_SECONDS}, GETDATE())
        """
    )
    ep_counts = fetch_one(
        """
...
def api_dashboard_stats():
    ...
    recent_logs = fetch_all(
        "SELECT TOP 8 * FROM traffic_logs ORDER BY log_time DESC, log_id DESC"
    )
    ...
    latest_alert = fetch_one(
        "SELECT TOP 1 * FROM alerts ORDER BY alert_time DESC, alert_id DESC"
    )
    if latest_alert:
...
def alerts():
    ...
    all_alerts = fetch_all(
        ...
        FROM alerts
        ORDER BY alert_time DESC, alert_id DESC
        OFFSET {offset} ROWS FETCH NEXT {ALERTS_PAGE_SIZE} ROWS ONLY
        """
    )
    ...
    blocked_ips = fetch_all(
        ...
        OFFSET {ip_offset} ROWS FETCH NEXT {IP_PAGE_SIZE} ROWS ONLY
        """
    )
...
def reports():
    report_rows = fetch_all(
        """
        SELECT TOP 50 r.*, u.username AS generated_by_name
        FROM reports r
...
def download_pdf():
    logs = fetch_all(
        """
        SELECT TOP 40 log_time, source_ip, destination_ip, port, protocol, action_type
        FROM traffic_logs
        ORDER BY log_time DESC, log_id DESC
        """
    )
...
def download_csv():
    ...
    logs = fetch_all(
        """
        SELECT TOP 500 log_time, source_ip, destination_ip, port, protocol, action_type
        FROM traffic_logs
        ORDER BY log_time DESC, log_id DESC
        """
    )
...
def api_agent_heartbeat():
    ...
    execute(
        """
        UPDATE endpoints
        SET last_seen = GETDATE(), status = ?, ip_address = ?, mac_address = ?, agent_version = ?
        WHERE endpoint_id = ?
        """,
...
def endpoints():
    ...
    return render_template(
        ...
        server_lan_url=f"http://{get_lan_ip()}:5000",
    )
...
if __name__ == "__main__":
    _mode_source = "FIREGUARD_APPLY_FIREWALL env var" if os.getenv("FIREGUARD_APPLY_FIREWALL") is not None else "fireguard_settings.json"
    print(
        f"\n[FireGuard] Starting in {'LIVE' if APPLY_FIREWALL_RULES else 'RECORDED'} mode "
        f"(source: {_mode_source}). If this isn't what you expected, start via "
        ".\\start_live_admin.ps1 instead of running app.py directly.\n"
    )
